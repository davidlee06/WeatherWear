...
fetch("/api/locker-image-info", {credentials: "same-origin"}).then((fetchResponse) => {
    fetchResponse.json().then((array) => {
        for (let a = 0; a < array.length; ++a) {
            images_root.innerHTML += `<div id="image-${a}"><h3 style="margin-top: 10px; margin-bottom: 10px;">Outfit #${a + 1}: Time created: ${
                array[a]["time_created"]
            }</h3><img src = "/api/get-image?image_id=${array[a]["id"]}"/><button class = "btn btn-primary remove_button"
